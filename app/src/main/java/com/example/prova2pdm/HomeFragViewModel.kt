...
class HomeFragViewModel (application: Application) : AndroidViewModel(application) {
    ...
    private inner class FindAllAsync() : AsyncTask<Unit, Unit, LiveData<List<Ator>>>() {
        override fun doInBackground(vararg params: Unit?): LiveData<List<Ator>> {
            return dba.AtorDAO().listAll()
        }
    }
}